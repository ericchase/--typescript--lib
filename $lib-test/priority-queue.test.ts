...
function buildBasicTests(
    ...
    for (
        const [key, permutation] of nChooseRPermutations(numbers, count)
            .entries()
    ) {
...
function buildComplexTests(
    ...
    isOrdered: (a: { v: number }, b: { v: number }) => boolean,
    count: number,
    expectedMapper: (expected: { v: number }[]) => { v: number }[] = (
        expected,
    ) => expected,
) {
    ...
    for (
        const [key, permutation] of nChooseRPermutations(objects, count)
            .entries()
    ) {
        it(`${key + 1}: ${permutation.join(",")}`, () => {
            for (const p of permutation) {
