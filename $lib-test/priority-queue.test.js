import { MaxPriorityQueue, MinPriorityQueue } from '../$lib/$adt/priority-queue';
import { nChooseRPermutations } from '../$lib/combinatorics';
...
function buildBasicTests(Queue, count, expectedMapper = (expected) => expected) {
  const queue = new Queue();
  const numbers = [...new Array(count).keys()];
  const expected = expectedMapper([...numbers]);
  for (const [key, permutation] of nChooseRPermutations(numbers, count).entries()) {
    it(`${key + 1}: ${permutation.join(',')}`, () => {
...
function buildComplexTests(Queue, isOrdered, count, expectedMapper = (expected) => expected) {
  const queue = new Queue(isOrdered);
  const objects = [...new Array(count).keys()].map((_) => ({
    v: _,
  }));
...
describe('MinPriorityQueue | Complex Object', () => {
  ...
  describe('Duplicate Items', () => {
    ...
    test('1,2,2,3', () => {
      let queue = new MinPriorityQueue(isOrdered);
...
describe('MaxPriorityQueue | Complex Object', () => {
  ...
  describe('Duplicate Items', () => {
    ...
    test('1,2,2,3', () => {
      let queue = new MaxPriorityQueue(isOrdered);
      queue.insert({ k: 0, v: 1 });
      queue.insert({ k: 1, v: 2 });
      queue.insert({ k: 2, v: 2 });
      queue.insert({ k: 3, v: 3 });
