import { zip } from '../src/lib/Array/zip.js';
/*
...
*/
function getSuccessiveProductsLeftToRight(array) {
  let product = 1;
  return [1, ...array.map((n) => ((product *= n), product))];
}
function getSuccessiveProductsRightToLeft(array) {
  let product = 1;
  return [1, ...[...array].reverse().map((n) => ((product *= n), product))].reverse();
}
function arrayProductExcludingElement(array) {
  return (
    Array.isArray(array) &&
